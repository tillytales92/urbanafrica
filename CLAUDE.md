...
| Piece | Status | Notes |
|---|---|---|
| "Footprint growth" mode with play slider (`app/app.R`) | **done (needs live-session testing)** | The old single-year res/nres snapshot is replaced by a World-Bank-atlas-style view: each pixel coloured by the 5-year epoch it was **first** built up (`terra::which.lyr(res+nres > 0)`, 1–6, viridis, discrete legend). `yr_single` is an animated `sliderInput`; a step just masks the epoch raster to `≤ year`. `output$map` renders a bare shell once; three observers push everything via `leafletProxy` — city (view + outline + info box), layers/legend (mode-keyed), and raster (per slider step). "Period change" mode unchanged. `first_built()` is computed on the fly per city and cached by the reactive; no new data needed. Not yet exercised live — check first paint, `which.lyr`, `colorFactor`+`addRasterImage`, and animation smoothness on big metros. |
